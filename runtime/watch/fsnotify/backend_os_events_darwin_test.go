...
func TestOSEventsBackend_Darwin_StartStop(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
func TestOSEventsBackend_Darwin_DetectsChange(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	if err := os.WriteFile(path, []byte("x: 1"), 0644); err != nil {
...
func TestOSEventsBackend_Darwin_InvalidPath(t *testing.T) {
	t.Parallel()
	b := newBackend()
